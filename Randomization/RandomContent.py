from lorem import LoremGenerator, get_word, get_paragraph, get_sentence, _TEXT  # https://github.com/JarryShaw/lorem#usage
import random
import typing
import itertools
from array import *
...
class RandomContent: #Standarize contents as dictionaries
	...
	def links(count_range, length_range, range_in_chars=False): 
		...
		if range_in_chars:
			...
			return list(itertools.islice(RandomContent.lorem_generator.sentence(count=n_elements,word_range=(length_range), comma=(0,0)), n_elements))
	
	def title(length_range, range_in_chars=False): 
		if range_in_chars:
			return list(itertools.islice(RandomContent.equi_text(1, length_range), 1))
		else:
			return get_word(count=random.randint(length_range[0], length_range[1]), func="capitalize") #TODO: Fix and test
	...
	def credits(length_range, range_in_chars=False): 
		if range_in_chars:
			...
			return get_word(count=random.randint(length_range[0], length_range[1]), func="capitalize")
	...
	def random_dictionary(count_range): #TODO: customize parameters
		...
		for i in range(n_items):
			d[get_word()] = get_paragraph(sentence_range=(1,3))
		return d
	...
	def titles_descriptions(count_range): #TODO: customize parameters
		...
		n_items = random.randint(count_range[0], count_range[1])
		for i in range(n_items):
			result_items.insert(0,[get_sentence(),get_paragraph()])
		return result_items
	...
	def equi_text(count, char_range):
		...
		n_chars_longest_word = len(max(_TEXT, key=len))
		...
		n_chars_shortest_word = len(min(_TEXT, key=len))
		n_pool_items = len(_TEXT)

		for _ in range(count):
			pool = RandomContent.lorem_generator._gen_pool()
			text = get_word(count=1, func="capitalize")
	...
	def matrix(n_rows, n_cols): #TODO: Recieve content range as parameter
		...
		for i in range(0, n_rows):
			for j in range(0, n_cols-1):
				content[i][j] = get_word()
		return content

	def form_input():
		r_name = get_word(count=random.randint(1,3))
		r_name = r_name[0].upper() + r_name[1:]
		r_desc = get_word(count=random.randint(3,6))
	...
	def input_choices(n_choices):
		...
		for i in range(n_choices):
			r_choice = get_word(count=random.randint(1,3))
